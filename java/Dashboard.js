  ...
  for (let i = 0; i < eventContainer.length; i++) {
    cartEv += `<tr>
        ...
        <td>
          ...
          <button class="btn btn-sm btn-outline-info ms-2" onclick="showOnMap('${eventContainer[i].location}')">
...
async function showOnMap(location) {
  try {
    const resolve = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(location)}&limit=1`);
...
function checkData() {
  if (eventsName.value != "" && eventDate.value != "" && eventTime.value != "" && eventPrice.value != "" && eventLocation.value != "" && eventsDescription.value != "") {
    return true;
  } else return false;
}
...
function searchEvents(searchInput) {
  ...
  for (var i = 0; i < eventContainer.length; i++) {
    if (eventContainer[i].name.toLowerCase().includes(searchInput.toLowerCase()) == true || eventContainer[i].date.toLowerCase().includes(searchInput.toLowerCase()) == true || eventContainer[i].location.toLowerCase().includes(searchInput.toLowerCase()) == true || eventContainer[i].price.toLowerCase().includes(searchInput.toLowerCase()) == true)
      cart += `<tr>
     <td>${i + 1}</td>
            <td>${eventContainer[i].date}</td>
            <td>${eventContainer[i].time}</td>
            <td> <div class="description">
            <a data-bs-toggle="collapse" href="#aIConference2025" role="button" aria-expanded="false" aria-controls="aIConference2025">${eventContainer[i].name}</a>
                <div class="collapse" id="aIConference2025">
                  <div class="card card-body description-card">
                    ${eventContainer[i].description}
                  </div>
                </div>
