  ...
  for (var i = 0; i < eventContainer.length; i++) {
    if (
     ...
     <td>${i + 1}</td>
            ...
            <a data-bs-toggle="collapse" href="#aIConference2025" role="button" aria-expanded="false" aria-controls="aIConference2025">${
              eventContainer[i].name
            }</a>
...
function displayAllEvents() {
  ...
  for (let i = 0; i < eventContainer.length; i++) {
    cartEv += `<tr>
        ...
        <td>
          ${eventContainer[i].location}
          <button class="btn btn-sm btn-outline-info ms-2 mt-2" onclick="showOnMap('${
            eventContainer[i].location
          }')">
